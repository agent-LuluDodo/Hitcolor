    ...
    public static class MidnightConfigScreen extends Screen {
        ...
        public void init() {
            ...
            ButtonWidget done = this.addDrawableChild(ButtonWidget.builder(ScreenTexts.DONE, button -> {
                ...
                write(modid);
                HitcolorConfig.updateColor();
                HitcolorConfig.updateHitcolor();
                Objects.requireNonNull(client).setScreen(parent);
    ...
    public static class ButtonEntry extends ElementListWidget.Entry<ButtonEntry> {
        ...
        public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            if (text != null && (!text.getString().contains("spacer") || !buttons.isEmpty())) {
                if (info.centered) DrawableHelper.drawTextWithShadow(matrices, textRenderer, text, Math.round(MinecraftClient.getInstance().getWindow().getScaledWidth() / 2f - (textRenderer.getWidth(text) / 2f)), y + 5, 0xFFFFFF);
